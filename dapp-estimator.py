...
class CodeReviewEstimator:
    ...
    def generate_report(self, extension_counts: Dict[str, int]) -> str:
        ...
        DAILY_RATE = 2000  # $2,000 per 8-hour day
        ...
        report.extend([
            ...
            f"Base Code Review Time: {code_review_hours:.1f} hours ({code_review_hours / 8:.1f} days)",
            f"Base Code Review Cost: ${(code_review_hours / 8 * DAILY_RATE):,.2f}"
        ])
        
        if self.additional_checks:
            report.append("\nAdditional Checks:")
            additional_check_days = 0
            for check in self.additional_checks:
                check_days = check.hours / 8
                additional_check_days += check_days
                check_cost = check_days * DAILY_RATE
                report.append(f"  {check.name}: {check.hours:.1f} hours ({check_days:.1f} days) (${check_cost:,.2f})")
                report.append(f"    Description: {check.description}")
            
            report.append(f"\nAdditional Checks Total Time: {additional_hours:.1f} hours ({additional_check_days:.1f} days)")
            report.append(f"Additional Checks Total Cost: ${additional_check_days * DAILY_RATE:,.2f}")
        
        report.extend([
            f"\nTotal Estimation Summary:",
            f"Total Review Time: {code_review_hours + additional_hours:.1f} hours ({total_days:.1f} days)",
